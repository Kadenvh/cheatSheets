...
- [ ] Configure knowledge-verifier agent (IDENTITY.md, SOUL.md)
- [ ] Verifier can: scan quick-inserts/ + processed/, compare against ChromaDB, report drift
- [ ] Reprocess command: wipe ChromaDB → re-run all raw sources through curator → re-embed
- [ ] Add Health tab integration: "Reprocess All" button that triggers verifier
...
- [ ] Create knowledge-learning agent (workspace, IDENTITY.md, SOUL.md, MEMORY.md, USER.md)
- [ ] SOUL.md: coverage tracking, gap analysis from Graph Connections, learning path curation
- [ ] Redesign Learn tab Overview: coverage dashboard + learning agent contact point
- [ ] Replace hardcoded LEARNING_PATH with server-persisted, agent-updated structure
- [ ] New API endpoint for learning path CRUD (or extend existing learning progress API)
...
- [ ] Create knowledge-demo agent (workspace, IDENTITY.md, SOUL.md)
- [ ] Add demo agent chat interface within Demos sub-tab
- [ ] Demo agent receives context from learning agent about user's current level
- [ ] Demo agent can generate exercises, quizzes, explanations, flash card reviews
- [ ] On session completion: demo agent generates cheatsheet-format output → curator → ChromaDB
- [ ] Review cycle: topics marked "needs review" → demo agent runs quiz → cheatsheet progress report → curator
- [ ] Generic DemoRenderer component for agent-designed exercises (JSON/markdown → rendered UI)
...
- [ ] Learning agent can initiate study sessions (provides pre-configured generation prompt)
- [ ] Learning agent observes user patterns and updates USER.md preferences
- [ ] Sessions sub-tab: session history, start new session, paste output workflow
- [ ] Learning style adaptation: visual vs. text, pace, depth preferences
