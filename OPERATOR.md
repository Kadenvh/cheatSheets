...
1. **The substrate is the continuity.** Your memory across sessions = the `brain.db` DAL (live + canonical, decision #19) + `SPEC.md` + `DECISIONS.md` + `PLAN.md` + `plans/` + `sessions/` + `CLAUDE.md` + this file + git history + Claude auto-memory. brain.db is the working memory exemplar; the `.md` files are its curated exports. Treat writes to these as the only mechanism of evolution.
...
| 2026-06-08 | 18 | **Three Session-17 calls reversed** (#19-21): DAL **unfrozen** (live + the working memory exemplar; `.md` files are its curated exports), **Mermaid-first** for design diagrams (Graphviz deferred to generated/data-dense views), **two-folder topology** (`cortex-design` houses the stalled learning-system; empty `cortex` build target). | Kaden, on reflection: a functional DAL is the best reference example of the memory system being designed (freeze was ideological); Graphviz had been adopted not chosen and leaned on a custom GUI; an empty third folder is build-ahead-of-need. Course-correction is allowed and expected (this contract's "Disagreement with prior sessions is allowed"). |
