...
19 skills total: 18 invocable via `/command`, 1 always-on (non-invocable).

## How Skills Work

Each skill lives at `.claude/skills/<name>/SKILL.md` and reads its protocol from `.prompts/<name>.md`. The SKILL.md is the entry point; the prompt file is the source of truth for behavior. When you type `/skill-name`, Claude loads the SKILL.md, which directs it to the corresponding prompt.
